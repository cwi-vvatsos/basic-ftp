...
class Client {
     ...
     */
    async downloadTo(destination, fromRemotePath, startAt = 0, rateLimit = 0) {
     ...
     * @protected
     */
    async _downloadToFile(localPath, remotePath, startAt, rateLimit) {
     ...
     * @protected
     */
    async _downloadToStream(destination, remotePath, startAt, rateLimit) {
        const onError = (err) => this.ftp.closeWithError(err);
